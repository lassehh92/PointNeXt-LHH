...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_path', type=str, required=False, help='path to the pre-trained model checkpoint', default=None)
    parser.add_argument('--freeze_base', type=int, help='Whether to freeze the base layers during fine-tuning', default=1)
    ...
    # init log dir
    cfg.task_name = args.cfg.split('.')[-2].split('/')[-2]  # task/dataset name, e.g., s3dis, modelnet40_cls
    cfg.cfg_basename = args.cfg.split('.')[-2].split('/')[-1]  # cfg_basename, e.g., pointnext-xl
    tags = [
        ...
        f'seed{cfg.seed}',
        'fine-tuning'  # Add a tag to indicate fine-tuning
    ]
